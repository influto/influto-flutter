...
class InfluToPurchaseObserver {
  ...
  final Map<String, ProductDetails> _oneTimeProducts =
  ...
  Future<PurchaseSyncResult> enable({bool backSync = true}) async {
    ...
    if (!InfluTo.instance.autoCaptureEnabled) {
      _log(
          'auto-capture not enabled (app is not store-direct, or autoCapture:false) — no-op');
      return PurchaseSyncResult.empty;
    }
    // SK2 is the default on iOS 15+; be explicit so we get the JWS, not the SK1 receipt.
    // (A harmless no-op where SK2 is already the default; the call is deprecated for that reason.)
    if (Platform.isIOS) {
  ...
  Future<PurchaseSyncResult> syncExisting() async {
    ...
    final r = PurchaseSyncResult(
        fetched: restored.length, sent: sent, failed: failed);
